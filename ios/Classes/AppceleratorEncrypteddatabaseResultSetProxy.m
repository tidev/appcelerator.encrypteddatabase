...
@implementation AppceleratorEncrypteddatabaseResultSetProxy
...
- (id)initWithResults:(EncPLSqliteResultSet *)results_ database:(AppceleratorEncrypteddatabaseDBProxy *)database_ pageContext:(id<TiEvaluator>)context
{
...
- (BOOL)isParseableString:(NSString *)str ofType:(AppceleratorDatabaseFieldType)type
{
  ...
  case TiFieldTypeDouble: {
    double v;
    return ([scanner scanDouble:&v] && [scanner isAtEnd]);
    break;
  }
  case TiFieldTypeFloat: {
    float v;
...
- (id)_transformObject:(id)obj toType:(AppceleratorDatabaseFieldType)type
{

  if (TiFieldTypeString == type) {
    return [TiUtils stringValue:obj];
  }

  id result = nil;

  if (TiFieldTypeInt == type) {
    BOOL valid = NO;
  ...
  if (TiFieldTypeFloat == type || TiFieldTypeDouble == type) {
    BOOL valid = NO;
    BOOL isString = [obj isKindOfClass:[NSString class]];
    if (!isString || (isString && [self isParseableString:obj ofType:type])) {
      result = (TiFieldTypeFloat == type) ? NUMFLOAT([TiUtils floatValue:obj def:0.0 valid:&valid])
                                          : NUMDOUBLE([TiUtils doubleValue:obj def:0.0 valid:&valid]);
    }
    if (!valid) {
      [self throwException:TiExceptionInvalidType subreason:[NSString stringWithFormat:@"Couldn't cast from %@ to %@", [obj class], (TiFieldTypeFloat == type) ? @"float" : @"double"] location:CODELOCATION];
    }
  }
  return result;
}
